...
@Service
public class CPUUsageJob implements Job {
    ...
    @Autowired
    public CPUUsageJob(KafkaTemplate<String, KafkaDomain> kafkaTemplate) {
        if (CPUUsageJob.kafkaTemplate == null) {
            CPUUsageJob.kafkaTemplate = kafkaTemplate;
            CPUUsageJob.TOPIC="cpuUsage";
        }
    }
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        ResponseEntity<GenericMetrics<Double>> response
                = restTemplate.exchange(dataMap.get("url")+"/cpuusage"+
