...
@Service
public class WarThreadJob implements Job {
    ...
    @Autowired
    public WarThreadJob(KafkaTemplate<String, KafkaDomain> kafkaTemplate) {
        if (WarThreadJob.kafkaTemplate == null) {
            WarThreadJob.kafkaTemplate = kafkaTemplate;
        }
        WarThreadJob.TOPIC="warThread";
    }
