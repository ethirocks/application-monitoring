...
@Service
public class WarRAMConsumer {
    private static KafkaTemplate<String, Alert> kafkaTemplate;
    private static String TOPIC;
    private MetricsService metricsService;
    private RamComparator ramComparator;
    @Autowired
    public WarRAMConsumer(MetricsService metricsService, RamComparator ramComparator, KafkaTemplate<String, Alert> kafkaTemplate) {
        this.metricsService = metricsService;
        this.ramComparator = ramComparator;
        if (WarRAMConsumer.kafkaTemplate == null) {
            WarRAMConsumer.kafkaTemplate = kafkaTemplate;
        }
        WarRAMConsumer.TOPIC="alert";
    }
    ...
    @KafkaListener(topics = "warRam", containerFactory = "kafkaListenerContainerFactory")
...
//                    System.out.println("Inside try");
    ...
    @KafkaListener(topics = "warRamLive", containerFactory = "kafkaListenerContainerFactory")
    public boolean consumeMetricsLive ( KafkaDomain message) {
        ...
        int alertLevel;
        long ramTotalSample = (long)(metricsService.searchMetrics("warRamTotalSampling",userId,applicationId,"threshold").getResults().get(0).getSeries().get(0).getValues().get(0).get(2));
