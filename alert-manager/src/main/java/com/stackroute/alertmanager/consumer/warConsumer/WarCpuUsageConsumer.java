...
@Service
public class WarCpuUsageConsumer {
    ...
    @Autowired
    public WarCpuUsageConsumer(MetricsService metricsService, UsageComparator usageComparator, KafkaTemplate<String, Alert> kafkaTemplate) {
        this.metricsService = metricsService;
        this.usageComparator = usageComparator;
        if (WarCpuUsageConsumer.kafkaTemplate == null) {
            WarCpuUsageConsumer.kafkaTemplate = kafkaTemplate;
        }
        WarCpuUsageConsumer.TOPIC="alert";
    ...
    @KafkaListener(topics = "warCpuUsage", containerFactory = "kafkaListenerContainerFactory")
    public boolean consumeMetrics(@Payload KafkaDomain message){
        ...
        try{
            ...
            Double meanCpuUsage =(Double)(metricsService.getMean("cpu_usage","warCpuUsage").getResults().get(0).getSeries().get(0).getValues().get(0).get(1));
            try{
//                    System.out.println("Inside try");
                Point cpuTempThreshold = Point.measurement("warCpuUsagethreshold")
        ...
        Double usageThreshold = (Double)(metricsService.searchMetrics("warCpuUsagethreshold",userId,applicationId,"threshold").getResults().get(0).getSeries().get(0).getValues().get(0).get(2));
        System.out.println("usagethresh"+usageThreshold);
        return true;
    }
    @KafkaListener(topics = "warCpuUsageLive", containerFactory = "kafkaListenerContainerFactory")
    public boolean consumeMetricsLive ( KafkaDomain message) {
        ...
        try {
            ...
            if (alertLevel>0){
                ...
                alert.setMetricsName("warCpuUsage");
